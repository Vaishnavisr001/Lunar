...
int main()
{
    ...
    auto i = mapEmp.begin();
    auto elef = i->first;//points to key
    auto ele = i->second;//points to the value ie now here vector
...
public:
    ...
    Emp(int i, const std::string& n, char g, int num)
        : id(i), name(n), gender(g), number(num) {}

    void disp() const {
        std::cout << id << ": {\"" << name << "\", '" << gender << "', " << number << "}" << std::endl;
    }
};

class EmpManager {
private:
    std::map<int, std::vector<Emp>> mapEmp;

public:
    // Add an employee to the vector mapped by key
    void addEmp(int key, const Emp& e) {
        mapEmp[key].push_back(e);
    }

    // Display all employees for a given key
    void displayEmps(int key) const {
        auto it = mapEmp.find(key);
        if (it != mapEmp.end()) {
            for (const auto& empObj : it->second) {
                empObj.disp();
            }
        }
        else {
            std::cout << "No employees found for key " << key << std::endl;
        }
    }
};
...
int main() {
    ...
    // Add employees to key 1
    manager.addEmp(1, Emp(101, "abc1", 'm', 10001));
    manager.addEmp(1, Emp(102, "abc2", 'f', 10002));
    manager.addEmp(1, Emp(103, "abc3", 'm', 10003));

    // Display employees for key 1
    manager.displayEmps(1);
