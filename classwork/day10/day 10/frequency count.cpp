...
int main()
{
	...
	cin >> num;
	long int temp;
	temp = num;//inorder not to loose the original value using temp
	makefreqcount(num);
	display();
	return 0;
}
...
{
	int remainder;
	cout << "In the make frequencyCount" << endl;
	while (temp)
	{
		remainder = temp % 10;
		switch (remainder)//hereb using switch to decrease the time complexity instead of using if else ladder
		{
		case 0:
			count0++;
			break;
